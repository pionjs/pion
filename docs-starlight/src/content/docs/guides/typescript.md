---
title: TypeScript
description: Learn how to use pion with TypeScript for type-safe Web Components.
---
...
function App({ userData }: AppProps) {
  return html`
    <img src=${userData.portrait} alt="user portrait" />
    <p>${userData.name}</p>
  `;
}
...
If you need to use `this` in your custom element, you can describe it as an HTMLElement or a shape that extends an HTMLElement.
